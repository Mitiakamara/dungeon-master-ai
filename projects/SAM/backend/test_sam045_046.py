...
_check_dice. Run: PYTHONUTF8=1 python3.14 test_sam045_046.py
...
CHAR = {"id": "b1", "name": "Björn", "stats": {"str": 18}, "status": {"attacks": [
    {"name": "Unarmed Strike", "bonus": "+7", "damage": "5"},   # fixed-damage from PDF
    {"name": "Greataxe", "bonus": "+7", "damage": "1d12+4"},
]}}

def arm(e, pending):
    e.set_pending("b1", "Björn", pending)

def armed_engine(c, weapon):
    e = MechanicEngine(c)
    arm(e, {"type": "weapon_attack", "weapon": weapon, "target": "Wolf",
            "target_data": c.initiative_order[0]})
    return e

# ── S1 — attack 1d20 → valid, resolves ──
print("S1 — 1d20 attack valid")
c = fresh_combat(); e = armed_engine(c, CHAR["status"]["attacks"][1])
...
check("pending preserved", e.get_pending("b1") is not None)
...
arm(e, {"type": "weapon_damage", "weapon": {"name": "X", "damage": "1d1+4"},
    "damage_spec": "1d1+4", "target": "Wolf", "target_data": c.initiative_order[0]})
r = e.process_player_roll(CHAR, {"dice": "1d4", "result": 3, "rolls": [3]})
...
# also garbage spec
arm(e, {"type": "weapon_damage", "weapon": {"name": "X", "damage": "xyz"},
...
c = fresh_combat(); e = MechanicEngine(c)
arm(e, {"type": "weapon_damage", "weapon": {"name": "Greataxe", "damage": "1d12+4"},
