-- Phase 10: Compendium Schema
-- Enable vector extension if not exists (already done, but good practice)
...
create table if not exists spells (
  ...
  source text default 'System', -- 'SRD', 'PHB', 'Homebrew'
  embedding vector(768) -- Google Embedding Dimension
);

-- Index for Spells
create index on spells using hnsw (embedding vector_cosine_ops);
...
-- Index for Monsters
create index on monsters using hnsw (embedding vector_cosine_ops);
create index idx_monsters_name on monsters(name);

-- 3. Items Table (Magic Items & Equipment)
create table if not exists items (
  id uuid primary key default gen_random_uuid(),
  name text not null,
  type text, -- Weapon, Armor, Potion
  rarity text, -- Common, Rare, Legendary
  description text,
  properties jsonb, -- {damage: "1d8", weight: "2lb"}
  source text default 'System',
  embedding vector(768)
);
...
-- 4. RAG Match Function for Compendium
-- Allows querying specific tables
...
returns table (
  id uuid,
  name text,
  content text, -- Generic content field for AI context
  similarity float
)
language plpgsql
as $$
