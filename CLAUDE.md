...
- **Combat system activation (23-24 Abr 2026):** Nuevo intent `start_combat` en `interpreter.py` (solo si `in_combat=False`, detecta agresión EN/ES). `orchestrator._handle_start_combat` hace monster lookup via `match_compendium` + re-query directo a tabla monsters (structured data: hp/ac/stats/actions), rola iniciativa para todos (PCs via DEX mod, NPC via compendium), emite `<DM_ROLL>` tags por combatiente (visible en frontend). Si primer turno es NPC o player delegado (`controlled_by`), auto-resuelve con `_resolve_npc_turns(advance_first=False)`. Delegated PCs ahora atacan NPCs (no aliados) vía `_build_combatant_from_character` que extrae ataques reales del `status.attacks`. HP updates funcionan correctamente vía `_flatten_player_for_combat` que hoista `status.hp_current/hp_max/ac` a top-level. Damage parser maneja damage type suffix (`"2d10 fire"`, `"1d6+2 slashing"`). NPC attacks emiten `<DM_ROLL>` tags (attack + damage). Narrator RULE 16 con referencia 5e de damage dice (Greataxe 1d12, Longsword 1d8, etc.) + cheat-sheet de armas inyectado en facts cuando es turno de un player. `COMBAT STATUS:` block (HP exacto de todos los combatientes) inyectado en cada turno — narrator cita verbatim, nunca inventa. `CombatState.to_dict()` incluye `current_turn: <name>` para compatibilidad frontend.
- **Player damage application (16 Abr 2026, bug crítico resuelto):** Durante combate, las tiradas del dice tray (d20 de ataque, non-d20 de daño) no aplicaban daño al HP del NPC — el interpreter clasificaba `[SYSTEM EVENT]` como `dice_roll` pero `engine.pending_player_roll` era `None`, y `process_player_roll` caía en `freeform_roll` sin side effects. **Fix:** Nuevo método `_setup_combat_freeform_pending()` en `orchestrator.py` que antes de `_handle_dice_roll`, si `combat.active` y no hay pending, infiere intent del dado: **d20** → `pending_player_roll = weapon_attack` contra primer NPC vivo usando `status.attacks[0]` (fallback: unarmed strike); **non-d20** → `pending_player_roll = weapon_damage` matcheando el dado vs `weapon.damage` (ej: d12 → greataxe). Los resolvers existentes (`_resolve_weapon_attack` / `_resolve_weapon_damage`) ya llamaban `combat.update_npc_hp()`, `remove_combatant` si HP ≤ 0, y `end_combat` si no quedan NPCs — solo faltaba que el flujo `dice_roll` los alcanzara. El turno del NPC corre **después** del damage roll (no del attack), vía `_resolve_npc_turns` cuando `pending_player_roll` queda limpio.
- **Admin command persistence (16 Abr 2026):** `/list`, `/checkpoint`, `/load`, `/gold`, `/memory`, `/delegate`, `/undelegate` ahora insertan su respuesta en `messages` table (`role=assistant`, `sender_id=NULL`, `visibility=public`) después de `AdminService.handle_command()` — así se broadcastean a toda la party via Realtime y quedan en el historial. `/reset` queda excluido (ya hace su propio broadcast con `CLEAR_CHAT`). Wrap defensivo en try/except — si el insert falla, el comando igual retorna al jugador.
- **Fetch robustness (16 Abr 2026):** `authenticatedFetch` (`lib/api.ts`) aborta tras 30s vía `AbortController` + `DEFAULT_TIMEOUT_MS = 30000` (protege contra mobile background hangs que colgaban el frontend indefinidamente). `handleSendMessage` valida `!res.ok` antes de parsear JSON para que 4xx/5xx caigan en el `catch` que ya mostraba toast con retry action. `dice-tray.tsx` muestra `toast.warning("⚠️ Dice rolled locally")` cuando `/api/roll` falla antes de caer al fallback `Math.random` — ya no hay tiradas locales silenciosas.
- **Turn enforcement + Extra Attack (24 Abr 2026, commit `839ba73`, SAM-002):** `orchestrator.process_message()` parsea el intent primero y luego corre un turn guard: si `combat.active` y `sender_name != current_turn.name`, bloquea intents `attack/spell/ability/start_combat` con un fact `OUT_OF_TURN:` (narrator produce sólo recordatorio in-character <30 palabras); para `dice_roll` permite sólo si `pending_player_roll.character_name == sender_name` (stamp agregado en `_setup_combat_freeform_pending`). Action economy en `combat_state.py`: nuevo helper `has_extra_attack(combatant)` (Barbarian/Fighter/Paladin/Ranger ≥ lvl 5 → 2, resto → 1; Fighter 11/20 simplificado), `actions_remaining` seeded por `_set_actions_for_current_turn()` en `start_combat` y `advance_turn`, `consume_action()` + `turn_is_over()` (`actions_remaining <= 0 and pending_action is None`), `actions_remaining` persistido en `to_dict()`. En el handler de `dice_roll`: snapshot de `previous_pending_type` antes de `_handle_dice_roll`, después si pending se limpió consume acción cuando previous ∈ {`weapon_attack`, `weapon_damage`, `spell_attack`, `spell_damage`} (cubre HIT + damage aplicado y también MISS), y si `turn_is_over()` corre `_resolve_npc_turns`, sino inyecta `"→ Björn has N action(s) remaining. Ask if they attack again."` en los facts. Player combatants ahora llevan `class` + `level` desde `_handle_start_combat`. Narrator RULE 16 con bullets nuevos para OUT_OF_TURN y Extra Attack invitation.
- **Initiative ground truth (24 Abr 2026, commit `738f85f`, SAM-013):** Narrator RULE 16 reemplaza el bullet genérico de iniciativa con "INITIATIVE GROUND TRUTH" — contrato de 4 sub-reglas: (1) preservar `<DM_ROLL>` verbatim, (2) prosa cita `result` exacto de cada tag, (3) turn order list cita números exactos, (4) empates siguen orden de los facts. Resuelve el bug donde narrator decía "la criatura se mueve con un 9" mientras el chip mostraba `result: 5`.
- **DM_ROLL hoist (24 Abr 2026, commit `c215cc7`, SAM-001):** `renderMessageContent` en `chat-interface.tsx` cuenta los `<DM_ROLL>` del mensaje — si hay 2+ entra en modo HOIST (extrae todos los chips con `matchAll(/<DM_ROLL>([\s\S]*?)<\/DM_ROLL>/g)`, los renderiza en un `<div flex flex-col gap-1 my-2 items-start>` al tope de la burbuja, y el texto narrativo va debajo sin tags y con whitespace colapsado: `[ \t]+→" "`, ` *\n→\n`, `\n{3,}→\n\n`). Con 0 o 1 chip mantiene el flujo inline clásico. Reemplaza el agrupador por whitespace del `73f8a38` (fallaba cuando Gemini intercalaba prosa corta entre tags).
- **Sistema de tickets (24 Abr 2026, commit `061f24a`):** `projects/SAM/SAM_tickets.md` trackea bugs/features/chores con IDs `SAM-XXX`, tipo (`BUG|FEAT|REFACTOR|CHORE`), prio (`P0` bloqueante → `P3` nice-to-have), estado (`OPEN|IN_PROGRESS|BLOCKED|DONE`). Workflow: ticket abierto antes de la instrucción, referenciado en el título ("Instrucción N — SAM-XXX título"), cerrado con commit + absorción de detalle en `SAM_progress_log.md`.
- **Completitud: ~97%** (app funcional) con arquitectura multi-agente + combat loop completo con turn enforcement + Extra Attack
- Ver `SAM_progress_log.md` para detalle completo y `SAM_tickets.md` para backlog activo
...
*Última actualización: 24 Abr 2026 — SAM: Turn enforcement + Extra Attack action economy (839ba73, SAM-002 DONE), initiative ground truth en narrator RULE 16 (738f85f, SAM-013 DONE), DM_ROLL hoist cuando hay 2+ rolls (c215cc7, SAM-001 DONE), sistema de tickets SAM_tickets.md (061f24a), DM_ROLL chips apilados verticalmente (73f8a38). Sesión previa 16 Abr: Player damage application, admin command persistence, fetch robustness (a8ced24 + 52e95de + 3a71c4e). Combat activation original: 6673cd0 + 255681d + 88c6d34 + d1613fa + d337241. FF8: Collections Module complete, SMS section in privacy policy, FF8 tickets backlog.*
