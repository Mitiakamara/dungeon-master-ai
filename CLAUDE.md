...
- **Auto-refresh character post-SAM:** Nuevo `useRealtime` en `messages` (filtered by campaign) que re-fetchea el character 1.5s después de cada respuesta de SAM. Belt-and-suspenders sobre el Realtime de `characters` table que a veces pierde fields nested al mergear.
- **Desktop polish del character sheet:** Bio & Gear tab apila vertical en desktop (`md:grid-cols-1`), inventory con full width + columnas rebalanceadas (`md:col-span-8/2/2`) + más padding (`md:px-3 md:py-2`). Spells table con `md:px-4 md:py-3`. Mobile sin cambios.
...
*Última actualización: 8 Abr 2026 — SAM: PDF parse hardening (65k tokens, thinking_budget=0, JSON mode, cleanup), automatic spell slot + inventory consumption, /gold command, auto-refresh character post-SAM, character sheet desktop polish. FF8: Collections Module complete.*
